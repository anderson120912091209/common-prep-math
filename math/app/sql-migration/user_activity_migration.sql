...
-- Function to update user activity when a problem is solved
...
-- Function to get user's contribution data for the last 365 days
...
COMMENT ON TABLE user_activity IS 'Tracks daily problem-solving activity for users to generate GitHub-style contribution graphs';
...
COMMENT ON FUNCTION increment_user_activity(UUID) IS 'Call this function when a user solves a problem to update their daily activity';
COMMENT ON FUNCTION get_user_contributions(UUID) IS 'Returns contribution data for the last 365 days for visualization';
COMMENT ON FUNCTION get_user_activity_stats(UUID) IS 'Returns total contributions, current streak, and longest streak for a user';
